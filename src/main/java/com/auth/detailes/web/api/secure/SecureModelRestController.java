...
import com.auth.detailes.web.mappers.ModelMapper;
...
@RestController
@AllArgsConstructor
@RequestMapping(EndPointConstent.MODEL_ENDPOINT_REST)
public class SecureModelRestController extends CustomRestController implements DefaultCrudOperations<ReferentialDTO, ReferentialSearchRequest> {
    ...
    public ApiResponse<Page<?>> findAllByCriteria(ReferentialSearchRequest searchRequest) {
        Page<Model> page = this.service.findAll(searchRequest, PageRequest.of(searchRequest.getPage(),searchRequest.getSize()));
