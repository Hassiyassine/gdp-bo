...
import com.auth.detailes.web.mappers.TypeDocumentMapper;
...
@RestController
@RequestMapping(EndPointConstent.TYPE_Document_ENDPOINT_REST)
@AllArgsConstructor
public class SecureTypeDocumentRestController  extends CustomRestController implements DefaultCrudOperations<TypeDocumentDTO, ReferentialSearchRequest> {
    ...
    public ApiResponse<Page<?>> findAllByCriteria(ReferentialSearchRequest searchRequest) {
        Page<TypeDocument> page = this.service.findAll(searchRequest, PageRequest.of(searchRequest.getPage(),searchRequest.getSize()));
        return new ApiResponse<>(Boolean.TRUE,null,page.map(mapper::toDTO),null);
    }

    @Override
    public ApiResponse<TypeDocumentDTO> findOneById(Long id) {
    ...
    @Override
    public ApiResponse<TypeDocumentDTO> add(TypeDocumentDTO form) {
        return new ApiResponse<>(Boolean.TRUE,null,mapper.toDTO(this.service.save(mapper.toEntity(form))),null);
    }

    @Override
    public ApiResponse<TypeDocumentDTO> update(Long id, TypeDocumentDTO form) {
